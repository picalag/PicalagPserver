...
public class StopWordsManager {
    ...
    private void loadList() {
        String[] swl = STOP_WORDS_LIST.split("|");
        for (String sw : swl) {
            stopWords.add(sw);
        }
    }
    ...
    public String removeStopWords(String text) {
        text = text.toLowerCase().replaceAll("\\b(" + STOP_WORDS_LIST + ")\\b", "").replaceAll("\\s+", " ");
        return text.trim();
    }
    ...
    public boolean isStopWord(String w) {
        boolean isSW = false;
        w = w.trim();
        if ((w == null) || (w.isEmpty()) || (stopWords.contains(w)) || (w.matches("\\d*"))) {
            isSW = true;
        }
        return isSW;
    }
    ...
    // removes both stop words and ponctuation
    public String cleanText(String text) {
        return removeStopWords(removePonctuation(text));
    }
}
